...
class DataSchemaAnalyzer(BaseTool):
    ...
    async def execute(self, **kwargs) -> Dict[str, Any]:
        ...
        if not data:
            return {
                ...
                "analysis": {},
                "suggestions": [],
                "summary": {"total_records": 0, "total_fields": 0},
            }
        ...
        try:
            ...
            return {
                ...
                "summary": {
                    ...
                    "nullable_fields": len(
                        [f for f in field_analysis.values() if f["has_nulls"]]
                    ),
    ...
    def _analyze_numeric_field(self, series: pd.Series) -> Dict:
        ...
        try:
            return {
                ...
                "potential_id_field": series.nunique() == len(series)
                and series.dtype in ["int64", "int32"],
            }
        except Exception:
    ...
    def _analyze_text_field(self, series: pd.Series, detect_categories: bool) -> Dict:
        ...
        try:
            ...
            analysis = {
                "avg_length": (
                    round(text_series.str.len().mean(), 2)
                    if not text_series.empty
                    else 0
                ),
                "min_length": (
                    text_series.str.len().min() if not text_series.empty else 0
                ),
                "max_length": (
                    text_series.str.len().max() if not text_series.empty else 0
                ),
                "has_special_chars": text_series.str.contains(
                    "[^a-zA-Z0-9\\s]", na=False
                ).any(),
            ...
            if detect_categories:
                ...
                analysis["is_categorical"] = (
                    unique_ratio < 0.5 and series.nunique() <= 50
                )
    ...
    def _analyze_datetime_field(self, series: pd.Series) -> Dict:
        ...
        try:
            return {
                ...
                "date_range_days": (
                    (series.max() - series.min()).days if not series.empty else 0
                ),
                "has_time_component": (
                    series.dt.hour.nunique() > 1 if not series.empty else False
                ),
                "timezone_aware": (
                    series.dt.tz is not None if not series.empty else False
                ),
            }
        except Exception:
    ...
    def _infer_semantic_type(self, series: pd.Series) -> str:
        ...
        if pd.api.types.is_numeric_dtype(series):
            if (
                ...
                and series.name
                and "id" in str(series.name).lower()
            ):
                return "identifier"
            ...
            sample_str = (
                str(series.dropna().iloc[0]) if not series.dropna().empty else ""
            )
            if "@" in sample_str:
                return "email"
    ...
    def _suggest_optimizations(self, field_analysis: Dict) -> List[Dict]:
        ...
        for col, analysis in field_analysis.items():
            ...
            if analysis.get("is_categorical") and analysis["data_type"] == "object":
                suggestions.append(
                    {
                        "type": "optimization",
                        "field": col,
            ...
            if (
                analysis["data_type"] in ["int64"]
                and "min_value" in analysis
                and "max_value" in analysis
            ):
            ...
            if (
                analysis["has_nulls"]
                and analysis["null_values"] / analysis["total_values"] > 0.8
            ):
                suggestions.append(
                    {
                        "type": "data_quality",
